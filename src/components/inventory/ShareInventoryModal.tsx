...
export default function ShareInventoryModal({ activeUnit, onClose, defaultLocation }: Props) {
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={customPin} onChange={(e) => setCustomPin(e.target.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(0, 6))}
                  ...
                  className="glass w-full rounded-xl px-4 py-3 text-sm text-text-primary outline-none placeholder:text-text-tertiary font-mono tracking-widest uppercase" maxLength={6} />
                <p className="mt-1 text-[10px] text-text-tertiary">6 caracteres alfanuméricos</p>
              </div>

              <button onClick={handleEnable} disabled={loading}
                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-500/20 transition-all hover:bg-emerald-600 active:scale-[0.98]">
                {loading ? <Loader2 size={16} className="animate-spin" /> : <Link2 size={16} />}
                Activar Enlace de Inventario
              </button>
            </div>
          ) : (
            /* ── Active state ── */
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-text-secondary">Enlace de Acceso</label>
                  <div className="flex items-center gap-1 rounded-md bg-white/5 p-0.5">
                    <button
                      onClick={() => setShareLang('es')}
                      className={`cursor-pointer rounded px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition-colors ${
